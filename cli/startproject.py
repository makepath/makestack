...
def startproject(name, directory):
    ...
    project_environment["django"] = qprompt.ask_yesno(msg="Add Django to the environment?")
    # project_environment["react"] = qprompt.ask_yesno(msg="Add React to the environment?")
    # project_environment["geoserver"] = qprompt.ask_yesno(msg="Add GeoServer to the environment?")
    # project_environment["mapshader"] = qprompt.ask_yesno(msg="Add Mapshader to the environment?")
    # project_environment["nginx"] = qprompt.ask_yesno(msg="Add Nginx to the environment?")

    if project_environment["django"]:
        project_environment["redis"] = qprompt.ask_yesno(msg="Add Redis to the environment?")
        project_environment["celery"] = qprompt.ask_yesno(msg="Add Celery to the environment?")
